...
def prim(source):
    # initialization of MST and Q (same values)
    # MST: an array of parent nodes in the MST, initially -1
    # Q: a min-heap queue contains elements of type (node, current_cost_to_MST)
    MST = [ -1 for i in range(n) ]
    MST[source] = source
    ...
    # process Q
    while Q:
	u, u_cost_to_MST = Q.popitem()
	# print 'popped ' + str(u+1) + ': ' + str(u_cost_to_MST)
	for v in graph[u]:
...
for i in range(m):
    ...
    existing_w = INFTY
    if v-1 in graph[u-1]:
        existing_w = graph[u-1][v-1]
    graph[u-1][v-1] = min(w, existing_w)
