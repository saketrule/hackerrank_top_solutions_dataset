...
public class Solution {
	...
	public void run() throws IOException {
		int T = Integer.parseInt(new String(io.nextLine()).trim());
//		int T = 1;
		while(T-- != 0) {
			int m = io.nextInt();
			int n = io.nextInt();
			int[] ys = io.nextIntArray(m - 1);
			int[] xs = io.nextIntArray(n - 1);
//			int[] ys = new int[m - 1]; Arrays.fill(ys, 1000000000);
//			int[] xs = new int[n - 1]; Arrays.fill(xs, 1000000000);
			radixSort(xs);
			radixSort(ys);
			for(int i = 0; i < (n - 1) / 2; i++) { swap(xs, i, n - 2 - i); }
			for(int i = 0; i < (m - 1) / 2; i++) { swap(ys, i, m - 2 - i); }
			long ans = 0;
			for(int i = 0, j = 0; i < n - 1 || j < m - 1; ) {
				while(i < xs.length && (j >= ys.length || xs[i] >= ys[j])) {
//					System.err.println(xs[i]);
					ans += xs[i++] * (j + 1L) % mod;
				}
				while(j < ys.length && (i >= xs.length || ys[j] >= xs[i])) {
//					System.err.println(ys[j]);
					ans += ys[j++] * (i + 1L) % mod;
				}
			}
